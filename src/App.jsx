...
const getSlotsDelDia = (slots, fechaKey) => {
...
function Sidebar({ activePage, onNav, pendingCount, mobileOpen, onMobileClose, onLogout }) {
  const { isMob } = useResp();
    ...
    <nav style={{
      ...
      <div style={{position:'relative',zIndex:1,marginTop:8}}>
        <div onClick={onLogout} style={{
          display:'flex',alignItems:'center',gap:9,padding:'10px 12px',borderRadius:10,cursor:'pointer',
          fontSize:13,color:'rgba(255,255,255,.6)',transition:'all .2s',
        }}>
...
function Dashboard({ clientes, turnos, onNav, onCompletar, onNoVino, onEditTurno }) {
  ...
  const stats = [
    {label:'Servicios Mes', val:serviciosMes, sub:'este mes', emoji:'🐶'},
    {label:'Servicios Año', val:serviciosAnio, sub:`año ${anioActual}`, emoji:'📅'},
    {label:'Ingresos Mes', val:fmtPeso(ingMesTotal), sub:`💵 ${fmtPeso(totalEfectivo)} | 📱 ${fmtPeso(totalTransferencia)}`, emoji:'💚'},
      ...
      <div style={{display:'grid',gridTemplateColumns:`repeat(${cols},1fr)`,gap:12,marginBottom:20}}>
        {stats.map(s => (
          <div key={s.label} style={{background:'white',borderRadius:16,padding:'16px 18px',boxShadow:'0 2px 8px rgba(0,0,0,.06)',position:'relative',overflow:'hidden'}}>
            ...
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:isMob?22:28,fontWeight:600,lineHeight:1.1,margin:'3px 0'}}>{s.val}</div>
...
function ModalCliente({ open, cliente, onClose, onSaveVisit, onEditVisit, onDeleteVisit, onDelete, onEdit, onDecrementarInasistencia }) {
  ...
  useEffect(() => {
    if (open) { setShowForm(false); setSvc(''); setPrecio(''); setFecha(todayStr()); setFormaPago('efectivo'); setEditingVisita(null); }
  }, [open]);
  ...
  const handleSaveVisita = () => {
    ...
    setShowForm(false); setSvc(''); setPrecio(''); setFecha(todayStr()); setFormaPago('efectivo');
  };

  const startEditVisita = (v) => {
    setEditingVisita(v);
    setSvc(v.servicio); setPrecio(String(v.precio)); setFecha(v.fecha); setFormaPago(v.forma_pago || 'efectivo');
    setShowForm(true);
  };
    ...
    <Modal open={open} onClose={onClose}>
      ...
      <div style={{padding:'18px 22px'}}>
        ...
        <div style={{display:'flex',gap:8,marginTop:14,flexWrap:'wrap'}}>
          <Btn size="sm" onClick={() => { setShowForm(!showForm); setEditingVisita(null); setSvc(''); setPrecio(''); setFecha(todayStr()); setFormaPago('efectivo'); }}>
...
function ModalNuevoTurno({ open, onClose, onSave, onUpdate, clientes, defaultFecha, turnoEdit }) {
  ...
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({clientId:'',dog:'',owner:'',raza:'',tel:'',svc:'',fecha:defaultFecha||todayStr(),hora:'10:00',precio:'',formaPago:'efectivo',estado:'confirmed'});
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      setForm(f => ({...f, fecha:defaultFecha||todayStr(), clientId:'', dog:'', owner:'', raza:'', tel:'', svc:'', hora:'10:00', precio:'', formaPago:'efectivo', estado:'confirmed'}));
      setMode('exist');
    }
  ...
  return (
    <Modal open={open} onClose={onClose} width={480}>
      ...
      <div style={{padding:'18px 22px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:isMob?'1fr':'1fr 1fr',gap:12,marginBottom:10}}>
          <FormGroup label="Forma de pago">
            <select value={form.formaPago} onChange={e=>set('formaPago',e.target.value)} style={inputStyle}>
              <option value="efectivo">💵 Efectivo</option>
              <option value="transferencia">📱 Transferencia</option>
            </select>
          </FormGroup>
      ...
      <div style={{background:'white',borderRadius:16,padding:'18px 20px',boxShadow:'0 2px 8px rgba(0,0,0,.06)'}}>
        <div style={{display:'flex',gap:10,flexWrap:'wrap',marginBottom:16}}>
          ...
          <select value={mes} onChange={e=>setMes(e.target.value)} style={{border:'1.5px solid #ede8e8',borderRadius:50,padding:'9px 14px',fontFamily:"'Outfit',sans-serif",fontSize:13,outline:'none',background:'white'}}>
            ...
            {months.map(m=>{
              const [yy, mm] = m.split('-');
              return <option key={m} value={m}>{MESES[parseInt(mm)-1]} {yy}</option>;
            })}
          </select>
        </div>
          ...
          : isMob ? (
            ...
            <div style={{overflowX:'auto'}}>
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead>
                  <tr>{['Mascota','Dueño','Servicio','Fecha','Pago','Precio'].map(h=>(
                ...
                <tbody>
                  {filtered.map((v,i)=>(
                    <tr key={v.id||i} style={{transition:'background .15s'}}>
                      ...
                      <td style={{padding:'10px 14px',borderBottom:'1px solid #ede8e8',fontSize:13}}><Badge variant={v.forma_pago==='transferencia'?'blue':'green'}>{v.forma_pago || 'efectivo'}</Badge></td>
...
function HorariosPage({ horariosData, onSaveHorarios }) {
  ...
  const agregarSlot = (dia) => {
    ...
    setSlots(s => ({...s, [k]: [...new Set([...getSlotsDelDia(s, k), hora])].sort()}));
    ...
    <section>
      ...
      <div style={{display:'grid',gridTemplateColumns:`repeat(auto-fill,minmax(${isMob?'160px':'190px'},1fr))`,gap:10,marginBottom:24}}>
        {DIAS_SEMANA_HOD.map(dia => {
          ...
          const horasDelDia = getSlotsDelDia(slots, fechaKey);
        ...
        <div ref={previewRef} style={{
          ...
          {(() => {
            ...
            return activos.map((dia, idx) => {
              const diaDate = getDiaDate(dia);
              const horasDia = getSlotsDelDia(slots, toKey(diaDate));
