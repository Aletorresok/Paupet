...
const btnBorde = {display:'flex',alignItems:'center',justifyContent:'center',gap:6,height:46,padding:'0 14px',borderRadius:12,border:'1px solid #4E6B60',background:'transparent',color:'white',fontFamily:'inherit',fontSize:14,cursor:'pointer',flex:1,whiteSpace:'nowrap'};
...
export default function ProximoTurnoCard({ proximo, cliente: c, onCompletar, onNoVino, onEditTurno }) {
  ...
  return (
    <section aria-label="Próximo turno" style={{background:C.verdeProfundo,color:'white',borderRadius:20,padding:isMob?18:'22px 24px',display:'flex',flexDirection:isMob?'column':'row',gap:isMob?14:20,alignItems:isMob?'stretch':'center'}}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:8,flexShrink:0,minWidth:isMob?0:220}}>
        ...
        <div style={{display:'flex',gap:8}}>
          ...
          <button type="button" onClick={()=>onEditTurno(t)} style={btnBorde} aria-label="Editar turno"><Icon name="edit"/></button>
