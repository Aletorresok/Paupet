...
export default function DatosPerro({ cliente: c, onRestarInasistencia }) {
  ...
  return (
    <div style={{display:'grid',gridTemplateColumns:isMob?'1fr 1fr':'repeat(3,minmax(0,1fr))',gap:12}}>
      {datos.map(([l, v]) => (
        <div key={l} style={{display:'flex',flexDirection:'column',gap:2}}>
          <span style={{fontSize:12,color:C.tintaSuave}}>{l}</span>
          <span style={{fontSize:15,fontWeight:500}}>{v}</span>
        </div>
      ))}
      <div style={{display:'flex',flexDirection:'column',gap:2}}>
        <span style={{fontSize:12,color:C.tintaSuave}}>Faltó sin avisar</span>
        <span style={{fontSize:15,fontWeight:500,color:c.inasistencias ? C.rosa : C.tinta,display:'flex',alignItems:'center',gap:8}}>
